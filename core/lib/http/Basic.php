...
use core\lib\http\Httprequest;
use core\lib\http\Httpresponse;
use core\lib\http\Cgistream;

class Basic{
    /* 
     * 下列公共属性可以作为选项传递给构造函数：
     */    
    public $addr = '0.0.0.0';               // 监听IP地址
    public $port = 80;                      // 监听端口号    
    public $cgi_env = array();              // 将附加环境变量的关联数组传递到 php-cgi
    public $server_id = 'HTTPServer/0.1';   // HTTP响应的“Server”标头中使用的标识符字符串
    public $php_cgi = 'php-cgi';            // 路径到php-cgi，如果不是在路径        
    
    /* 
     * 活跃的客户端套接字的资源ID HttpRequest对象内部地图
     */    
    ...
    function run_forever(){    
        ...
        stream_wrapper_register("cgi", "core\lib\http\Cgistream");
