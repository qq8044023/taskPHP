...
class Cgistream{
    ...
    function stream_open($path, $mode, $options, &$opened_path){
        ...
        $php_cgi_path='';
        /* foreach (explode(':', $_SERVER['PATH']) as &$item){
            if(strpos($item,'php')){
                $php_cgi_path=$item.DS;
                continue;
            }
        } */
        
        $proc = proc_open($php_cgi_path.$php_cgi, $descriptorspec, $pipes, 
            __DIR__, 
            $cgi_opts['env'],
            array(
                'binary_pipes' => true,
        ...
        switch ($this->cur_state){
            case static::BUFFERING:
                ...
                if ($end_response_headers === false){
                    ...
                    $headers = Basic::parse_headers($headers_str);        
